...
interface AddClientDialogProps {
    ...
    onSave: (clientData: { name: string, email: string, phone: string }) => Promise<any>;
    onBookAppointment: (clientData: any) => void;
}

export const AddClientDialog = ({ open, onOpenChange, onSave, onBookAppointment }: AddClientDialogProps) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const newClient = await onSave(formData);
            setCreatedClient(newClient); // Store for potential booking
            setStep('success');
        } catch (error: any) {
            ...
            <DialogContent className="sm:max-w-[500px]">
                <DialogHeader>
                    <DialogTitle className="flex items-center gap-2">
                        ...
                        {step === 'form' ? "Registrar Nuevo Cliente" : "¡Cliente Registrado!"}
                    </DialogTitle>
                    <DialogDescription>
                        {step === 'form'
                            ? "Ingrese los datos del cliente para darlo de alta en el sistema."
                            : "El cliente ha sido registrado exitosamente. ¿Qué deseas hacer ahora?"}
                    </DialogDescription>
                </DialogHeader>
                ...
                {step === 'form' ? (
                    ...
                    <div className="py-6 space-y-6">
                        <div className="flex flex-col items-center text-center space-y-2">
                            ...
                            <h3 className="text-xl font-bold text-gray-900">{createdClient?.name}</h3>
                            <p className="text-gray-500">{createdClient?.email}</p>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            ...
                            <Button className="h-auto py-4 flex flex-col gap-2" onClick={() => {
                                handleClose();
                                onBookAppointment(createdClient);
                            }}>
